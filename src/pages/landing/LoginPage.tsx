  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signInError) {
        ...
        // Admins and Players both go to /arcade. 
        // ArcadePage handles role-based routing and session checks!
        navigate("/arcade");
      }
    } catch (err) {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px] px-4 py-12 relative overflow-hidden">
      ...
      <Card className="w-full max-w-md relative z-10 border-2 border-slate-200 border-b-[6px] border-b-indigo-500 rounded-2xl shadow-xl shadow-slate-200/50 bg-white/95 backdrop-blur-sm">
        <CardHeader className="text-center pb-6 pt-8">
          <div className="mx-auto mb-4 h-14 w-14 rounded-full bg-indigo-50 flex items-center justify-center text-3xl border-2 border-indigo-100 shadow-inner">
            🕹️
          </div>
          <CardTitle className="text-2xl font-black tracking-tight text-slate-900">Resume Game</CardTitle>
          <CardDescription className="text-slate-500 font-medium mt-2">
            Sign in to access the arcade and check your standings.
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="px-8 pb-8">
          <form onSubmit={handleSubmit} className="space-y-5">
            {error && (
              <div className="rounded-xl border-2 border-red-200 bg-red-50 p-4 flex items-start gap-3 text-red-600 animate-in fade-in slide-in-from-top-2" role="alert">
            ...
            <Button type="submit" size="lg" className="w-full mt-6 text-lg tracking-wide" loading={loading}>
              {loading ? "Authenticating..." : "Enter Arcade"}
            </Button>
          </form>
        </CardContent>
